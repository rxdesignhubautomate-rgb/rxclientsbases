...
provide an always-on process for the polling workers. On Vercel, accepted
WhatsApp webhooks are processed after the immediate `200` response using
Vercel's `waitUntil`; durable polling/retry workers still require an always-on
worker host. See [deployment-vercel.md](docs/deployment-vercel.md).
