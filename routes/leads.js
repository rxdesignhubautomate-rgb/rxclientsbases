...
import {
  ...
  failMarketingCampaign,
  listMarketingCampaigns,
} from "../services/marketingCampaignStore.js";
...
leadsRouter.get("/broadcast/history", async (req, res, next) => {
  try {
    res.json({ ok: true, ...(await listMarketingCampaigns(req.query.limit)) });
  } catch (error) {
    next(error);
  }
});
...
leadsRouter.post("/broadcast/send", async (req, res, next) => {
  ...
  try {
    ...
    const campaignTarget = bulkResult.phones.length
      ? "bulk_numbers"
      : targets.length === 1
        ? targets[0]
        : "multiple";
    if (marketingTemplate && !testPhone) {
      const campaign = await createMarketingCampaign({
        templateName: templateOptions.name,
        languageCode: templateOptions.languageCode,
        targets: bulkResult.phones.length ? [] : targets,
        target: campaignTarget,
        requested: candidates.length,
        createdBy: req.device?.user || req.device?.role || "admin",
      });
          ...
          await updateLead(lead.id, {
            ...
            ...(marketingTemplate
              ? {
                  ...
                  lastMarketingCampaignId: campaignId,
    ...
    if (campaignId) await completeMarketingCampaign(campaignId, result);
    if (streaming) {
    ...
    if (campaignId) {
      try {
        await failMarketingCampaign(campaignId, error);
      } catch (campaignError) {
        console.error("marketing_campaign_failure_save_failed", {
          campaignId,
