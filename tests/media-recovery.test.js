...
describe("WhatsApp media recovery", () => {
  it("recovers provider media, links it to the message, and serves its content", async () => {
    ...
    const refreshedConversation = await core.store.get("conversations", conversation.conversationId);
    expect(refreshedConversation.mediaUpdatedMessageId).toBe(saved.message.messageId);
    expect(refreshedConversation.mediaUpdatedAt).toBeInstanceOf(Date);
  });
