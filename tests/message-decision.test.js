...
describe("WhatsApp message decision policy", () => {
  ...
  it("selects Utility for a verified order confirmation outside 24 hours", () => {
    expect(decision({
  ...
  it("blocks an order event without orderId", () => {
    expect(decision({ eventType: "ORDER_CONFIRMATION", transactionVerified: true }).reason).toContain("MISSING_TRANSACTION_DATA");
  });
  ...
  it("uses an approved Utility event outside the service window", () => {
    expect(decision({ eventType: "READY_TO_DISPATCH", orderId: "ORD_REAL", transactionVerified: true }).mode).toBe("UTILITY_TEMPLATE");
  });
