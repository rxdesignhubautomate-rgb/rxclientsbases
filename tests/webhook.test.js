...
describe("durable WhatsApp webhook", () => {
  ...
  it("touches the conversation when Meta advances an outbound delivery status", async () => {
    const { core, service } = await setup();
    const inboundBody = payload("wamid.inbound");
    const inboundEvent = await service.receiveWhatsApp({
      rawBody: Buffer.from(JSON.stringify(inboundBody)),
      payload: inboundBody,
      signature: "test"
    });
    await service.processEvent(inboundEvent.webhookEventId);
    const conversation = (await core.store.find(COLLECTIONS.conversations, { limit: 1 })).items[0];
    await core.store.create(COLLECTIONS.messages, "MSG_OUTBOUND_STATUS", {
      messageId: "MSG_OUTBOUND_STATUS",
      orgId: "RXDH",
      ...
      entry: [{
        changes: [{
          value: {
            ...
            statuses: [{ id: "wamid.outbound", status: "read", timestamp: "1710000010" }]
          }
        }]
      }]
    };
    const statusEvent = await service.receiveWhatsApp({
      rawBody: Buffer.from(JSON.stringify(statusBody)),
      payload: statusBody,
      signature: "test"
    });
    await service.processEvent(statusEvent.webhookEventId);

    const message = await core.store.get(COLLECTIONS.messages, "MSG_OUTBOUND_STATUS");
    const updatedConversation = await core.store.get(COLLECTIONS.conversations, conversation.conversationId);
    expect(message.status).toBe("READ");
    expect(updatedConversation.deliveryStatusMessageId).toBe("MSG_OUTBOUND_STATUS");
    expect(updatedConversation.deliveryStatus).toBe("READ");
    expect(updatedConversation.deliveryStatusUpdatedAt).toBeInstanceOf(Date);
    expect(updatedConversation.updatedAt).toBeInstanceOf(Date);
  });
});
