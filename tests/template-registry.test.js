...
describe("Meta template registry", () => {
  it("syncs Meta status and permits only APPROVED templates", async () => {
    ...
    const service = new TemplateRegistryService({
      ...
      whatsappAdapter: {
        listMessageTemplates: async () => [
          { id: "T1", name: "quotation_ready_v1", language: "en", category: "UTILITY", status: "APPROVED" },
    ...
    await expect(service.assertApproved("RXDH", "QUOTATION_READY")).resolves.toMatchObject({ status: "APPROVED" });
