...
curl -X POST "https://YOUR_RENDER_HOST/api/v1/message/decide" \
  ...
  -d '{"leadId":"LEAD_DOCUMENT_ID","eventType":"ORDER_CONFIRMATION","orderId":"ORDER_DOCUMENT_ID","templateKey":"order_confirmation","templateData":{"customer_name":"Amit","order_reference":"ORD-1025","order_value":"INR 12,500"}}'
```
...
```bash
curl -X POST "https://YOUR_RENDER_HOST/api/v1/events/order-confirmed" \
  -H "Authorization: Bearer FIREBASE_ID_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"leadId":"LEAD_DOCUMENT_ID","orderId":"ORDER_DOCUMENT_ID","customerName":"Amit","orderValue":"INR 12,500"}'
```

Design approved:

```bash
curl -X POST "https://YOUR_RENDER_HOST/api/v1/events/design-approved" \
  -H "Authorization: Bearer FIREBASE_ID_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"leadId":"LEAD_DOCUMENT_ID","orderId":"ORDER_DOCUMENT_ID","customerName":"Amit"}'
```

Ready to dispatch:

```bash
curl -X POST "https://YOUR_RENDER_HOST/api/v1/events/ready-to-dispatch" \
  -H "Authorization: Bearer FIREBASE_ID_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"leadId":"LEAD_DOCUMENT_ID","orderId":"ORDER_DOCUMENT_ID","customerName":"Amit"}'
```
...
```bash
curl -X POST "https://YOUR_RENDER_HOST/api/v1/events/experience-feedback" \
  -H "Authorization: Bearer FIREBASE_ID_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"leadId":"LEAD_DOCUMENT_ID","orderId":"ORDER_DOCUMENT_ID","customerName":"Amit"}'
```
...
Backend v2.6.0 returns an actionable error instead of a generic `Internal server error` when Meta or Firestore rejects a sync. The message distinguishes:
