...
GET    /api/v1/contacts/:contactId/overview
...
`overview` returns the permanent client record plus summary totals, orders,
payments, quotations, follow-ups, conversations, attachments, and leads.

## Existing-client imports

```text
POST /api/v1/imports/order-register/preview
POST /api/v1/imports/order-register/commit
```

Both accept `{ sourceName, headers, rows }`, where `headers` is an array and
`rows` is an array of cell arrays. Preview is read-only. Commit is idempotent and
