...
Up to 10,000 conversation summaries and 10,000 messages total are cached in the browser's IndexedDB, with a 500-message maximum for any single conversation. Oldest cached records are evicted automatically. The inbox renders that cache immediately after sign-in, then requests only changed conversations and new messages every 60 seconds while the tab is visible. After two empty checks it waits two minutes. Conversation reconciliation and cached templates, quick replies, users, and capabilities refresh once every 24 hours; the Refresh control can request an immediate reconciliation. The cache contains message metadata, not media file bytes; Firebase Storage remains the durable source for images and documents.
