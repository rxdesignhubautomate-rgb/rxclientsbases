...
export class OtpAuthService {
  ...
  async verifyOtp(rawEmail, rawCode) {
    ...
    const user = await this.provisionAccount(account);
    const token = this.tokenGenerator();
    const sessionId = digest(token);
    const expiresAt = new Date(Date.now() + this.env.OTP_SESSION_HOURS * 60 * 60_000);
    const finalization = await this.store.runTransaction(async (tx) => {
      const otp = await tx.get(COLLECTIONS.loginOtps, otpId);
      if (!otp || otp.consumed || (toDate(otp.expiresAt)?.getTime() || 0) < Date.now()) {
        return new AppError("OTP_INVALID_OR_EXPIRED", "OTP is invalid or expired", 401);
      }
      if (!safeEqual(otp.codeHash, this.hashOtp(email, code))) {
        return new AppError("INVALID_OTP", "Incorrect OTP", 401);
      }
      await tx.update(COLLECTIONS.loginOtps, otpId, { consumed: true, consumedAt: new Date() });
      await tx.set(COLLECTIONS.loginSessions, sessionId, {
        userId: user.userId,
        orgId: user.orgId,
        email: user.email,
        createdAt: new Date(),
