...
export function decideMessageType(input = {}) {
  ...
  if (serviceWindow.open) {
    return allowed(MESSAGE_MODES.SERVICE, "The recorded customer-service window is open", base, null, false);
  }
...
function utilityTemplateKey(templateKey, eventType) {
  ...
  const match = Object.entries(getWhatsAppTemplateRegistry())
    .find(([, template]) => template.category === "UTILITY" && template.eventType === eventType);
  return match?.[0] || null;
}
