...
export class ContactService {
  ...
  async overview(orgId, contactId) {
    const contact = await this.get(orgId, contactId);
    const filters = [["orgId", "==", orgId], ["contactId", "==", contactId]];
    const [orders, quotations, followUps, conversations, payments, attachments, leads] = await Promise.all([
      this.store.find(COLLECTIONS.orders, { filters, limit: 250 }),
      this.store.find(COLLECTIONS.quotations, { filters, limit: 250 }),
      this.store.find(COLLECTIONS.followUps, { filters, limit: 250 }),
      this.store.find(COLLECTIONS.conversations, { filters, limit: 250 }),
      this.store.find(COLLECTIONS.payments, { filters, limit: 500 }),
      this.store.find(COLLECTIONS.attachments, { filters, limit: 250 }),
      this.store.find(COLLECTIONS.leads, { filters, limit: 250 })
    ]);
    const orderItems = sortNewest(orders.items, "orderDate");
    const receivedPayments = payments.items.filter((payment) => payment.status === "RECEIVED");
    const totalValue = orderItems.reduce((sum, order) => sum + Number(order.totalAmount || 0), 0);
    ...
    return {
      ...
      summary: {
        ...
        totalValue,
        paidAmount,
        outstandingAmount: Math.max(0, totalValue - paidAmount),
        openFollowUps: followUps.items.filter((item) => item.status === "SCHEDULED").length,
        openConversations: conversations.items.filter((item) => item.status !== "CLOSED").length
      },
      orders: orderItems,
      quotations: sortNewest(quotations.items),
      followUps: sortNewest(followUps.items, "dueAt"),
      conversations: sortNewest(conversations.items, "lastMessageAt"),
      payments: sortNewest(payments.items, "receivedAt"),
      attachments: sortNewest(attachments.items),
      leads: sortNewest(leads.items)
    };
  }
...
function sortNewest(items, preferredField = "createdAt") {
  return [...items].sort((left, right) => {
    const a = new Date(left[preferredField] || left.createdAt || 0).getTime() || 0;
    const b = new Date(right[preferredField] || right.createdAt || 0).getTime() || 0;
    return b - a;
  });
}
