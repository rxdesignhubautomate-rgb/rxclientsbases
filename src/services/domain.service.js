...
export class DomainService {
  ...
  async list(resource, orgId, options = {}) {
    const cfg = resourceConfig(resource);
    ...
    const result = await this.store.find(cfg.collection, {
      filters,
    ...
    const allowed = new Set(contacts.filter(item => item?.orgId === orgId && options.relationshipTypes.includes(item.relationshipType || 'PROSPECT')).map(item => item.contactId || item.id));
    return { ...result, items: result.items.filter(item => allowed.has(item.contactId)) };
  }
  ...
  async touchConversations(orgId, contactId) {
    if (!contactId) return;
    const result = await this.store.find(COLLECTIONS.conversations, { filters: [['orgId','==',orgId],['contactId','==',contactId]], limit: 100 });
    await Promise.all(result.items.map(item => this.store.update(COLLECTIONS.conversations,item.conversationId || item.id,{updatedAt:now()})));
  }
