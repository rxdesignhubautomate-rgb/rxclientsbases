  ...
  async createInbound(input) {
    ...
    return this.store.runTransaction(async (tx) => {
      ...
      tx.update(COLLECTIONS.conversations, input.conversationId, {
        ...
        inboundSequence: message.inboundSequence,
        messageCount: Number(conversation?.messageCount || 0) + 1,
        lastMessageAt: timestamp,
        lastMessagePreview: preview(input.text, input.type),
        lastInboundAt: new Date(Math.max(toSafeDate(conversation?.lastInboundAt)?.getTime() || 0, Math.min(toSafeDate(input.providerTimestamp)?.getTime() || timestamp.getTime(), timestamp.getTime()))),
  ...
  async updateProviderStatus(orgId, providerMessageId, status, error = null, providerTimestamp = null, providerMetadata = null) {
    ...
    await this.store.runTransaction(async (tx) => {
      ...
      if (current.conversationId) {
        tx.update(COLLECTIONS.conversations, current.conversationId, {
          deliveryStatusMessageId: messageId,
          deliveryStatus: patch.status,
          deliveryStatusUpdatedAt: statusAt,
          updatedAt: now()
        });
      }
