...
function leadEventPatch(eventType) {
  if (["DESIGN_PROOF_READY", "DESIGN_APPROVAL_PENDING"].includes(eventType)) return { designStatus: eventType };
  if (["PRINTING_STARTED", "BINDING_STARTED", "ORDER_READY"].includes(eventType)) return { productionStatus: eventType };
  if (["ORDER_DISPATCHED", "TRACKING_UPDATED", "DELIVERY_UPDATED"].includes(eventType)) return { dispatchStatus: eventType };
  if (["PAYMENT_RECEIVED", "PAYMENT_DUE", "REFUND_UPDATED"].includes(eventType)) return { paymentStatus: eventType };
  return {};
}
