...
export class SmartMessageService {
  ...
  async decide(orgId, input, actor = {}) {
    const context = await this.loadContext(orgId, input);
    if (actor.role) await assertInboxAccess(this, orgId, context.conversation || context.contact, actor);
  ...
  async loadContext(orgId, input) {
    ...
    if (lead && lead.contactId !== contactId) throw new ConflictError("Lead does not belong to the selected contact");
    ...
    if (input.conversationId) {
      ...
      conversation = result.items
        .filter((item) => item.orgId === orgId && (item.currentChannel || item.channel) === "WHATSAPP")
  ...
  async ensureConversation(orgId, contact, requestedConversationId = null) {
    ...
    let identity = identities.items.find((item) => (item.currentChannel || item.channel) === "WHATSAPP" && item.active === true);
    if (!identity) {
