...
const schema = z.object({
  ...
  CRM_DIRECTORY_ENABLED: booleanFromString,
  CRM_MARKETING_UPGRADE_ENABLED: booleanFromString,
  CRM_MARKETING_DISPATCH_ENABLED: booleanFromString,
  CRM_INACTIVITY_DAYS: z.coerce.number().int().min(1).max(3650).default(90),
  ...
  CAMPAIGN_POLL_INTERVAL_MS: z.coerce.number().int().min(30000).default(60000),
  CAMPAIGN_BATCH_SIZE: z.coerce.number().int().min(1).max(100).default(100),
  ...
  LEGACY_JOBS_ENABLED: z.enum(["true", "false"]).default("false").transform((v) => v === "true"),
