...
export function createApp(options = {}) {
  ...
  app.get("/health", (_req, res) => {
    res.json({ status: "ok", service: "rx-communication-crm", version: "2.2.0", timestamp: new Date().toISOString() });
  });
  app.get("/ready", async (_req, res) => {
    try {
      await container.store.get("systemSettings", "readiness");
      res.json({ status: "ready", service: "rx-communication-crm", version: "2.2.0", timestamp: new Date().toISOString() });
    } catch {
