import { customerServiceWindow } from "../services/conversation.service.js";
...
export class OutboundWorker {
  ...
  async tick() {
    ...
    this.running = true;
    try {
      const stale = await this.store.find(COLLECTIONS.outbox, {
        filters: [["status", "==", "PROCESSING"], ["lockedAt", "<=", new Date(Date.now() - 5 * 60_000)]], orderBy: ["lockedAt", "asc"], limit: this.batchSize
      });
      for (const record of stale.items) {
        await this.store.runTransaction(async tx => {
          const current = await tx.get(COLLECTIONS.outbox, record.outboxId || record.id);
          if (current?.status !== 'PROCESSING' || toDate(current.lockedAt)?.getTime() > Date.now() - 5 * 60_000) return;
          const message = await tx.get(COLLECTIONS.messages, current.messageId);
          const delivered = Boolean(message?.providerMessageId);
          tx.update(COLLECTIONS.outbox, current.outboxId || record.id, { status: delivered ? 'SENT' : 'DELIVERY_UNKNOWN', lockedAt: null, lockedBy: null, updatedAt: now() });
          if (!delivered && message) tx.update(COLLECTIONS.messages, current.messageId, { status: 'DELIVERY_UNKNOWN', errorCode: 'DELIVERY_UNKNOWN', errorMessage: 'Worker interrupted during delivery. Check provider delivery before sending again.', updatedAt: now() });
        });
      }
  ...
  async processOne(candidate) {
    ...
    const claimed = await this.store.runTransaction(async (tx) => {
      ...
      if (!current || !due || !["PENDING", "RETRY"].includes(current.status) || (current.status === "PROCESSING" && !stale)) return null;
  ...
  async assertSendAllowed(message) {
    const [contact, conversation] = await Promise.all([
      this.store.get(COLLECTIONS.contacts, message.contactId),
