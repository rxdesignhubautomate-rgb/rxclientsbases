  ...
  async processOne(candidate) {
    ...
    try {
      await this.media.markMessageMediaState(claimed.orgId, claimed.payload.messageId, "DOWNLOADING")
        .catch((error) => safeWarn(this.logger, {
          error: error.message,
      ...
      safeWarn(this.logger, {
        automationJobId: id,
        messageId: claimed.payload?.messageId,
        providerMediaId: claimed.payload?.media?.providerMediaId,
      ...
      await this.media.markMessageMediaState(
        ...
        final ? "FAILED" : "RETRY",
        details.message
      ).catch((stateError) => safeWarn(this.logger, {
...
function safeWarn(logger, fields, message) {
  try {
    ...
    // Logging must never change download or retry behaviour.
  }
}
