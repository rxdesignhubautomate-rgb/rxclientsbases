...
export async function recordMessage(input) {
  ...
  return db.runTransaction(async (tx) => {
    ...
    let marketingCampaignReplyRef = null;
    if (
      input.type !== "reaction" &&
    ...
    if (message.role === "user" && input.type !== "reaction") {
      ...
      if (previouslyAwaiting && timestamp >= marketingSentAt) {
        ...
        const campaignId = String(lead.lastMarketingCampaignId || "").trim();
        if (
          campaignId &&
          String(lead.marketingReplyCampaignId || "") !== campaignId
        ) {
          patch.marketingReplyCampaignId = campaignId;
          marketingCampaignReplyRef = db
    ...
    tx.set(ref, message);
    if (marketingCampaignReplyRef) {
      tx.set(
        marketingCampaignReplyRef,
        {
